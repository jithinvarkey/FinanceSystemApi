...
/**
 * P3.6–P3.10 — Vendor invoices. Draft -> approval (P0.4) -> post to GL
 * (Dr expense/asset + Dr input VAT 15% → Cr vendor payable). Duplicate guard
 * on (vendor, vendor's own invoice number).
 */
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('vendor_invoices', function (Blueprint $table): void {
            $table->id();
            $table->string('invoice_number', 30)->unique()->comment('System, e.g. VINV-2026-000001');
            $table->string('vendor_invoice_no', 60)->nullable()->comment("Vendor's own invoice number");
            $table->foreignId('vendor_id')->constrained('vendors')->restrictOnDelete();
            $table->date('invoice_date');
            $table->date('due_date')->nullable();
            $table->string('reference', 100)->nullable()->comment('PO / external reference');
        ...
        Schema::create('vendor_invoice_lines', function (Blueprint $table): void {
            ...
            $table->unique(['vendor_invoice_id', 'line_no']);
        });
    }
