...
/**
 * P4.6–P4.10 — Customer (sales / commission) invoice.
 *
 * @property CustomerInvoiceStatus $status
 */
final class CustomerInvoice extends Model implements Approvable
{
    use HasApprovals;
    use HasAttachments;
    use SoftDeletes;
    ...
    protected $fillable = [
        ...
        'subtotal', 'tax_amount', 'total_amount', 'amount_paid', 'status', 'is_opening', 'fiscal_period_id',
    ...
    /** Posted and still owing — eligible for a receipt. */
    public function isReceivable(): bool
    {
        return $this->status === CustomerInvoiceStatus::Posted && $this->balanceDue() > 0;
    }

    /** @return HasMany<CustomerInvoiceLine> */
    public function lines(): HasMany
    {
        return $this->hasMany(CustomerInvoiceLine::class)->orderBy('line_no');
    }

    /** @return BelongsTo<Customer, CustomerInvoice> */
    ...
    public function onApprovalRejected(ApprovalRequest $request): void
    {
        ...
        $this->forceFill([
            'status' => CustomerInvoiceStatus::Rejected,
